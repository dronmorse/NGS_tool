...
function LimitByHitlistSize(list)
{
    ...
    var maxNumSeq;
    if(!$("maxNumSeq")) {
        maxNumSeq = getHiddenFieldVal("MAX_NUM_SEQ");
        maxNumSeq = (maxNumSeq && maxNumSeq != "") ? maxNumSeq : undefined;
    }
    else {
        maxNumSeq = $("maxNumSeq").value;
    }
    if(!maxNumSeq) return;
    for(i=0; i < list.options.length; i++) {    
        var optVal = parseInt(list.options[i].value ,10);
        var hitListSize = parseInt(maxNumSeq,10);                
...
function adjustFormatOptions() 
{
    ...
    var dynFormatQA = queryAnch;
    dynFormatPW = formatType[formatType.selectedIndex].value == "HTML" && (alignView[alignView.selectedIndex].value == "Pairwise" || alignView[alignView.selectedIndex].value == "PairwiseWithIdentities");    
