...
function initAlignDownLoad(navObj) 
{
    ...
    if(dwnFSTW) dwnFSTW.setAttribute("seqfsta", navObj.seqFSTADwnld); 
    if(dwnFSTAl) dwnFSTAl.setAttribute("seqfsta", navObj.seqFSTADwnld); 
...
function initDownLoadFSTA(radioElem, descr) 
{
    if (descr) {
        ...
        var tblID = "#dscTable,#dscTable_psiw";
        
        $("sbmtFASTA").ALIGN_SEQ_LIST.value = createSelseqStringFSTA(tblID,"seqFSTADwnld");        
    }
    else {        
...
function createSelseqStringFSTA(tblID,seqAttrName) {    //tblID= "#dscTable,#dscTable_psiw"
    var selSeqs = "";
    jQuery(tblID).find("input[type='checkbox'].cb").each(function(index) {
        if (jQuery(this)[0].checked) {
            if (selSeqs != "") selSeqs += ",";                        
            trObj = jQuery(this).closest('tr');                
            var linkID = "deflnDesc_" + jQuery(trObj).attr("ind");            
            selSeqs += $(linkID).getAttribute(seqAttrName);            
        }
    });
    return selSeqs;
}
...
function initNavInfo(navObj) {
    ...
    if ($("deflnDesc_" + currInd)) {
        ...
        navObj.seqFSTADwnld = $("deflnDesc_" + currInd).getAttribute("seqFSTADwnld");        
